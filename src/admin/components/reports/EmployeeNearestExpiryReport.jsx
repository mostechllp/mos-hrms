...
const EmployeeNearestExpiryReport = () => {
  ...
  // Fetch employee nearest expiry report
  useEffect(() => {
    dispatch(
      fetchEmployeeNearestExpiryReport({
        page: currentPage,
  ...
  const handleExport = async (format) => {
    ...
    if (exportReport.fulfilled.match(result)) {
      ...
      showToast(`Employee nearest expiry report exported successfully!`, "success");
    } else {
  ...
  return (
    <div className="w-full overflow-x-hidden">
      ...
      <ExportModal
        ...
        subtitle={`Exporting employees with documents expiring within ${expiryDays} days`}
      />
    </div>
  );
};
