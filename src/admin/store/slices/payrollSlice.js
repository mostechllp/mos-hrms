      ...
      if (contentDisposition) {
        ...
        for (const pattern of patterns) {
          ...
          if (match && match[1]) {
            ...
            if (candidate && candidate !== "payslip.pdf") {
              filename = candidate;
              console.log("✅ Found filename using pattern:", pattern, "→", filename);
              break;
            }
          }
        }

        // If still using default, try manual parsing
        if (filename === "payslip.pdf") {
          const parts = contentDisposition.split(";");
          for (const part of parts) {
            const trimmed = part.trim();
            if (trimmed.startsWith("filename=")) {
              let val = trimmed.substring(9);
              val = val.replace(/^["']|["']$/g, "");
              if (val) {
                filename = val;
                console.log("✅ Found filename via manual parsing:", filename);
                break;
              }
            }
            if (trimmed.startsWith("filename*=")) {
              let val = trimmed.substring(10);
...
// ✅ FIXED: Fetch payroll entries - with year and optional month
...
const payrollSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // ─── Convert Salary ────────────────────────────────────────────────
      .addCase(convertSalary.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(convertSalary.fulfilled, (state, action) => {
        state.loading = false;
        state.summaryData = {
          ...state.summaryData,
          converted: action.payload,
        };
        state.successMessage = "Salary converted successfully";
      })
      .addCase(convertSalary.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload;
      })
      ...
      .addCase(fetchWorkingDays.fulfilled, (state, action) => {
        state.loading = false;
        // Store working days data in stepData step 1
        state.stepData[1] = {
          ...state.stepData[1],
      ...
      // ─── Export Payrolls ──────────────────────────────────────────────
      .addCase(exportPayrolls.pending, (state) => {
        state.actionLoading = true;
        state.error = null;
      })
      .addCase(exportPayrolls.fulfilled, (state) => {
        state.actionLoading = false;
        state.successMessage = "Payrolls exported successfully";
      })
      .addCase(exportPayrolls.rejected, (state, action) => {
        state.actionLoading = false;
        state.error = action.payload;
      })

      // ─── Send Payslip ──────────────────────────────────────────────────
      .addCase(sendPayslip.pending, (state) => {
        state.actionLoading = true;
        state.error = null;
      })
      .addCase(sendPayslip.fulfilled, (state, action) => {
        state.actionLoading = false;
        state.successMessage = action.payload?.message || "Payslip sent successfully";
      })
      .addCase(sendPayslip.rejected, (state, action) => {
        state.actionLoading = false;
        state.error = action.payload;
      })
