...
const OffboardingDashboard = () => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      showToast(
        `Offboarding for ${selectedOffboarding.name} deleted successfully`,
        "success",
      );
      setConfirmDeleteOpen(false);
      setSelectedOffboarding(null);
      // Slice already removes it from state.offboardings — no refetch needed.
      // But we also want to clean up the local progress map.
      setProgressData((prev) => {
        const next = { ...prev };
        delete next[selectedOffboarding.id];
        return next;
      });
    } catch (error) {
      showToast(error || "Failed to delete offboarding", "error");
    } finally {
      setDeleting(false);
    }
  };
  ...
  const offboardingCards = [
    {
      ...
      description:
        "Start the 6-stage offboarding SOP. Set last working day and trigger workflows.",
    ...
    <div className="w-full overflow-x-hidden p-6">
      <div className="stats-grid grid grid-cols-1 sm:grid-cols-3 gap-5 mb-6">
        {quickStats.map((stat, index) => (
          <div
            key={index}
            className="bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-200 dark:border-gray-700 shadow-soft"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        {offboardingCards.map((card) => (
          <div
            key={card.id}
            className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 shadow-soft cursor-pointer hover:-translate-y-1 transition-all"
            onClick={() => navigate(card.path)}
          >
            <div
              className={`w-14 h-14 ${card.bgClass} rounded-xl flex items-center justify-center mb-4`}
            >
              {card.icon}
            </div>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-gray-700">
              <button
                className={`px-4 py-2 rounded-lg ${card.buttonClass} font-semibold text-sm flex items-center gap-2 transition-all`}
              >
      ...
      <ConfirmModal
        ...
        title="Delete Offboarding"
        message={`Are you sure you want to delete the offboarding record for "${selectedOffboarding?.name}"? This action cannot be undone.`}
        confirmText="Delete"
        loading={deleting}
        variant="danger"
      />
