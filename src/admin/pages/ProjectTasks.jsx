...
const Tasks = () => {
  ...
  const { tasks = [], loading, error, totalCount, currentPage, lastPage, perPage } = 
  ...
  }, [dispatch, currentPageState, perPageState, searchTerm, statusFilter, projectFilter, departmentFilter]);
  ...
  const getTaskStatus = (task) => {
    ...
    if (task.assigned_to && task.assigned_to.length > 0 && task.assigned_to[0].pivot?.status) {
  ...
  const getStatusBadge = (status) => {
    const statusMap = {
      ...
      assigned: { label: "Assigned", class: "bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400" },
    };
    const s = statusMap[status] || { label: status || "Unknown", class: "bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300" };
    ...
    <div className="w-full overflow-x-hidden">
      ...
      <div className="stats-grid grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-4 mb-6">
        <div className="bg-white dark:bg-gray-800 rounded-xl p-3 md:p-4 border border-gray-200">
          ...
          <div className="text-[10px] md:text-xs text-gray-500">Total Tasks</div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-xl p-3 md:p-4 border border-gray-200">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-3 md:p-4 border border-gray-200">
          ...
          <div className="text-[10px] md:text-xs text-gray-500">In Progress</div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-xl p-3 md:p-4 border border-gray-200">
          ...
          <div className="text-xl md:text-2xl font-bold text-yellow-600">{stats.onHold}</div>
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        ...
        <select
          ...
          onChange={(e) => { setDepartmentFilter(e.target.value); setCurrentPageState(1); }}
          className="px-3 md:px-4 py-2 bg-white dark:bg-gray-800 border border-gray-200 rounded-full text-xs md:text-sm"
        >
        ...
        <button
          ...
          className="px-3 md:px-4 py-2 bg-white border border-gray-200 rounded-full text-sm flex items-center gap-2"
        >
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700">
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400">Task</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400">Project</th>
                        ...
                        <tr className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                          ...
                          <td className="px-4 py-3">
                            <div>
                              ...
                              {task.task_description && (
                                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 line-clamp-1">{task.task_description}</p>
                              )}
                            </div>
                          </td>
                          <td className="px-4 py-3">
                            <span className="text-sm text-gray-700 dark:text-gray-300">
                              {taskProject?.project_name || taskProject?.name || "-"}
                            </span>
                          </td>
                          <td className="px-4 py-3">
                            <div className="flex flex-wrap items-center gap-2">
                              {assignedEmployees.length > 0 ? (
                                assignedEmployees.map((emp, i) => {
                                  ...
                                  const empName = emp.first_name || emp.employee_name || emp.name || 'Unknown';
                                  return (
                                    <div key={i} className="flex items-center gap-2">
                                      ...
                                      <span className="text-sm text-gray-700 dark:text-gray-200">{empName}</span>
                                    </div>
                                  );
                                })
                              ) : (
                          ...
                          <td className="px-4 py-3">
                            {hasMultipleEmployees ? (
                              ...
                              >
                                <span>{isStatusExpanded ? 'Hide Statuses' : 'Show Statuses'}</span>
                                <i className={`fas fa-chevron-${isStatusExpanded ? 'up' : 'down'} text-[10px] transition-transform duration-200`}></i>
                              </button>
                            ) : (
